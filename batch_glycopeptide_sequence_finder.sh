...
mkdir -p digested_peptide_library
mkdir -p digested_glycopeptide_library
mkdir -p logs

# Generate timestamp for this run
timestamp=$(date +%Y%m%d_%H%M%S)
log_file="logs/batch_run_${timestamp}.log"
...
for fasta_file in ${input_dir}/*.fasta; do
    ...
    output_file="digested_glycopeptide_library/$(basename ${fasta_file} .fasta)_${protease}_digested_mc${missed_cleavages}_z${charge_state}_${glycosylation_type}-glycopeptides.csv"
    
    # Run with error handling
    if python glycopeptide_sequence_finder_cmd.py \
        -i "${fasta_file}" \
        -o "${output_file}" \
        -p ${protease} \
        -g ${glycosylation_type} \
        -c ${missed_cleavages} \
        -l "logs/$(basename ${fasta_file} .fasta)_${timestamp}.log" \
        -v \
        -z ${charge_state} \
...
# Count sequences
peptide_count=$(count_entries "digested_peptide_library")
glycopeptide_count=$(count_entries "digested_glycopeptide_library")

# Create summary report
summary_report="logs/summary_batch_run_${timestamp}.txt"
